# Set Up Panini Check Scanner

Source: https://rmxhelp.rentmanager.com/Topics/Express/KB/Checks-Scan-Panini-Set-Up.htm
...
Rent Manager allows you to scan and record multiple checks or money orders in a batch and apply them to tenant or prospect accounts from one page. In order to use the check scanning tool, you first need to acquire and set up a check scanning device. To set up your check scanner, you need to install and set up the Scan Manager application, install drivers for your scanner, and then activate the device so it can be used in Rent Manager .
 ...
 This topic covers setup instructions for the Panini Vision X50, but Rent Manager also supports the TellerScan TS 240 and Epson check scanners. For more information on setting up a TellerScan check scanner, refer to Set Up TellerScan Check Scanner . For more information on setting up an Epson check scanner, refer to Set Up Epson Check Scanner .
 ...
 The first step to setting up your check scanner is to download and install Scan Manager. Drivers for Epson scanners are installed while installing Scan Manager. Drivers for Panini scanners must be installed separately, as covered in the next step. To install Scan Manager, do the following:
...
The Scan Manager application is installed.
 ...
 For Scan Manager to detect your Panini scanner, you must download and install drivers for the device. To install the drivers, do the following:

 -
 In your web browser, go to the Tools page on the Rent Manager website.

 -
 In the Printing & Check Scanner Tools section, click Panini Universal Installer .
...
The unzipped folder displays.
 ...
 -
 Open the Panini Universal Installer folder, then double-click setup .

 More Information

 There are several items named setup in the folder. To install the driver, double-click the application-type item shown below:
