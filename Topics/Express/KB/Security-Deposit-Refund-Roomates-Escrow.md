# Refund a Security Deposit to Roommates from Escrow

Source: https://rmxhelp.rentmanager.com/Topics/Express/KB/Security-Deposit-Refund-Roomates-Escrow.htm

When a tenant and/or their roommate(s) moves out, you can use the security deposit refund tool to refund their security deposit or apply the deposit (partially or completely) to new or existing charges. You can also use this tool while they are still occupying the property to apply part of a held security deposit toward an open charge. Some state and municipal governments require that security deposits held accrue interest, which must be returned to tenants and roommates when you refund security deposits.
 ...
 This topic covers the process of refunding security deposits for roommates who are set as contacts on a tenant or prospect account. If your roommates are all set up with their own accounts and managed via an account group, instead follow the standard refund process for each account. For more information, refer to Refund a Security Deposit Held in Escrow .

 If the security deposits for the tenant's property are held in the property's operating bank account, the process of refunding a full or partial security deposit is different. For more information, refer to Refund a Security Deposit Between Roommates .
