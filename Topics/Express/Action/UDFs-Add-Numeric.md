# Add a Numeric User Defined Field

Source: https://rmxhelp.rentmanager.com/Topics/Express/Action/UDFs-Add-Numeric.htm

User defined fields (UDFs) can be created to track information that Rent Manager does not track by default, such as combinations for unit key lock boxes, the access code for a gated community, or the percentage of a bill to be paid by a property when disbursing amounts. You can create UDFs for many Rent Manager entities (e.g., prospects, assets, service issues, loans, etc.), with each entity having its own unique set of UDFs. While accounts of the same entity share the same set of UDFs, the values are unique to each account.

 There are several UDF types that can be created. This topic focuses on the Numeric field type.
 ...
 To create a new numeric UDF, do the following:
...
The User Defined Fields page displays.
 ...
 A brief description of the data that is tracked by the UDF (e.g., Key Lock Box Code ).
 ...
 Select Numeric from the drop-down list.

 More Information

 For more information on the other options in the Field Type drop-down list, refer to Add a User Defined Field , Add a List User Defined Field , Add an Encrypted User Defined Field , and Add a File/Image User Defined Field .

 Default Value

 A numerical value to be automatically populated when accounts of the selected entity type are created after the UDF is saved.

 # of digits after decimal

 The maximum number of places to the right of the decimal point a user can enter for this UDF. For example, if you enter 2 , users can enter up to two digits after the decimal point ( 0.12 ).
 ...
 Automatically applies the default value to all existing and future accounts of the selected type.

 No

 The UDF value remains blank for existing accounts of the selected entity type. Only future accounts populate with the default value.
