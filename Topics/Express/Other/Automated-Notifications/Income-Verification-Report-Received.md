# Income Verification Report Received (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Income-Verification-Report-Received.htm

The Income Verification Report Received automated notification can be configured to send an email or text message to property managers, other Rent Manager users, or external recipients when the income verification process is completed by AmRent for a tenant or prospect. This can be used to alert any relevant parties that they need to review the results and proceed with the remaining tasks associated with the verification request.

 Related Preferences

 In order to utilize this notification, you must have Income Verification credentials entered for screening products in system preferences. For more information, refer to Screening Products Credentials (System Preferences) .
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to inform managers at all properties when income verification reports are received, but notify leasing agents only for residential properties. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Income Verification Report Received .
The Automated Notification: Income Verification Report Received page displays.
...
The setup's details display.
 ...
 Include the manager of the property associated with the prospect or tenant whose income verification report was received. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
