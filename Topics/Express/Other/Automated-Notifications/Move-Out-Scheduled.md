# Move Out Scheduled (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Move-Out-Scheduled.htm

The Move Out Scheduled automated notification can be configured to send an email or text message to users, tenants, owners, or other external recipients as a tenant's move-out date approaches. This can be useful in ensuring that all necessary inspections and move-out reports are completed in a timely manner.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to alert the property manager and leasing agent at all properties when a move-out date is two weeks away, but notify tenants at multiple intervals. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Move Out Scheduled .
The Automated Notification: Move Out Scheduled page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 The number of days before a tenant's Move Out date that you want the automatic notification to send. For example, if you want alerts to be sent one week before a tenant is scheduled to move out, enter 7 .
 ...
 Include the manager of the property where the tenant is moving out. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
 ...
 If the selected setup is under Notify Tenant , you can configure this notification setup to be sent to the tenant whose move-out date is approaching and additional contacts associated with the tenant.
