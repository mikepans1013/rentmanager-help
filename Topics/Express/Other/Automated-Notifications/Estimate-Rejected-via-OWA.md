# Estimate Rejected via OWA (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Estimate-Rejected-via-OWA.htm

The Estimate Rejected via OWA automated notification can be configured to send an email or text message to the sales representative, other Rent Manager users, or external recipients when an owner estimate is rejected in Owner Web Access (OWA) . This is used to alert the sales representative that a new estimate needs to be created or that a follow-up is needed with the owner.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to alert sales representatives for all rejected estimates, but notify property managers only when estimates for commercial properties are rejected. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Estimate Rejected via OWA .
The Automated Notification: Estimate Rejected via OWA page displays.
