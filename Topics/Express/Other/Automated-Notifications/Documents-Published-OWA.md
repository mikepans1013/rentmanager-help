# Documents Published to OWA (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Documents-Published-OWA.htm

The Documents Published to OWA automated notification can be configured to send an email or text message to owners after a signable document is published to their Owner Web Access (OWA) account. This can be used to notify owners that a document is ready for review and signatures.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, owners of certain properties may have less time to sign a document before it expires, so a configuration can be enabled to send specific instructions to the owners of those properties. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Documents Published to OWA .
The Automated Notification: Documents Published to OWA page displays.
...
The setup's details display.
 ...
 For this notification , you can opt to exclude specific owners or any owners that have a specific user defined field (UDF) value.
