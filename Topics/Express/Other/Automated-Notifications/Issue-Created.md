# Issue Created (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Issue-Created.htm

The Issue Created automated notification can be configured to send an email or text message to users or external recipients when a new issue is created in Rent Manager or rmAppSuite Pro so they can follow up. You can also send alerts to tenants to confirm that their issue was created or to owners to keep them informed on the issues being worked on at your properties.

 More Information

 This automated notification generates alerts only for issues created via Rent Manager . For more information on automated notifications for issues created through Tenant Web Access (TWA) , refer to Issue Created via TWA (Automated Notification) .
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to have the assigned user notified whenever a service issue is created at certain properties, but also notify tenants at all properties only for issues with a specific category. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Issue Created .
The Automated Notification: Issue Created page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the owner(s) of the property linked to the created issue.
 ...
 Each service issue category that should generate alerts when issues in those categories are created and meet the automated notification's conditions.
