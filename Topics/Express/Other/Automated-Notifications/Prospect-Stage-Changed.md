# Prospect Stage Changed (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Prospect-Stage-Changed.htm

The Prospect Stage Changed automated notification can be configured to send an email or text message to prospects when a stage change has occurred. This can be used to inform prospects of where they are in the process of becoming a tenant. Additionally, you can configure this notification to notify property managers or Rent Manager users about stage changes that need their attention.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, property managers have a list of tasks to perform when a prospect moves to a new stage. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to   arrow_forward   Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Prospect Stage Changed .
The Automated Notification: Prospect Stage Change page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to prospect(s) who have entered a new Prospect Stage .
 ...
 The category that identifies where in the leasing process the prospect is. For example, if you want to inform prospects when their screening is completed or when they are added to a wait list, you can select user-created stages such as Screening Complete and Waitlisted . Prospects receive notifications only for the selected stages.

 Step 2: Configure Recipients

 The next step is to determine who receives this configuration of the notification, and whether they receive it by Email , Text , or Email and Text . You can choose to include one or multiple recipients. The available recipient options vary depending on the notification type, indicated by the Notification Setup section's drop-down headers.
 ...
 Include the manager of the property the prospect is associated with . Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
 ...
 If the selected setup is under Notify Prospect , you can configure this notification setup to be sent to the prospects as they progress through each Prospect Stage in the leasing process.
 ...
 In the Recipients tile header, click   arrow_forward   Exclude Prospect . Then on the Exclude Prospect pop-up, enter information into at least one of the available fields and click OK .
 ...
 For example, if you select a UDF named Receives Notifications? and then select a value of No , then prospects with No set as the value for that UDF do not receive this notification.
 ...
 If at least one recipient is configured to receive email notifications, the Email section displays with a default, system-generated message. The following fields are available in this section:
