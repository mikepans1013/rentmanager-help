# Issue Assigned to User (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Issue-Assigned-to-User.htm

The Issue Assigned to User automated notification can be configured to send an email or text message to users when a service issue is assigned to them. This can be used to notify users as soon as possible when a new issue is assigned to their username.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to notify only the users assigned to the issue at some properties, but at another property the property manager also wants to receive a notification. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Issue Assigned to User .
The Automated Notification: Issue Assigned to User page displays.
...
The setup's details display.
 ...
 Each issue category that should trigger this notification when an issue of that category is assigned to a user. To include all current and future categories, check *** < All Categories > .

 For example, if you select only one issue category named Maintenance , then only issues with that category selected trigger this notification. This notification is not sent for any issues with a different category selected, even when a user is assigned to the issue.
 ...
 Add Newly Assigned User

 Include the Rent Manager user who is now selected in the Assigned To User field of the issue's details.
