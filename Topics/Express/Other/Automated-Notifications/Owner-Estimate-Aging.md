# Owner Estimate Aging (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Owner-Estimate-Aging.htm

The Owner Estimate Aging automated notification can be configured to send an email or text message to sales representatives, other Rent Manager users, or owners when an owner estimate is a certain number of days old and has not expired or been set to a status of Approved or Rejected . This can be used to remind the owner that the offer on the estimate is waiting for their response. Sales representatives may also be notified to alert them that they may want to check in with the owner.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Owner Estimate Aging .
The Automated Notification: Owner Estimate Aging page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the owner associated with the aging estimate.
 ...
 The number of days after the owner estimate's Date when the notification should be sent. For example, if you would like the notification to be sent to when the estimate is ten days old, enter 10 .
 ...
 If the selected setup is under Notify Owner , you can configure this notification setup to be sent to the owner whose estimate is expiring.
