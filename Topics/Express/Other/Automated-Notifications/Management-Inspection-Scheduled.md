# Management Inspection Scheduled (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Management-Inspection-Scheduled.htm

The Management Inspection Scheduled automated notification can be configured to send an email or text message to users, tenants, owners, or other external recipients when a management inspection's scheduled date is approaching in Rent Manager or rmAppSuite Pro . This can be used to remind users that one of their assigned inspections needs to be completed in the near future or inform tenants that an inspection linked to their unit is coming up.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to notify only users with the Property Manager role one week before an inspection is scheduled at certain properties, but notify tenants at all properties one day before an inspection they are linked to is scheduled. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Management Inspection Scheduled .
The Automated Notification: Management Inspection Scheduled page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the tenant and, optionally, tenant contact(s) linked to the scheduled inspection.

 Owner

 Creates a notification setup that can be sent to the owner(s) of the property linked to the scheduled inspection.
 ...
 The number of days before the inspection's scheduled date that you want the automatic notification to send. For example, if you want alerts to be sent one week before an inspection's scheduled date, enter 7.
 ...
 Each inspection type that should generate alerts when inspections of those types are scheduled and meet the automated notification's conditions. To include all inspection types, check *** < All Types > .
 ...
 Include the user assigned to the inspection at the time it is scheduled.

 Add Property Manager

 Include the manager of the property linked to the scheduled inspection. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
