# Tenant Estimate Aging (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Tenant-Estimate-Aging.htm

The Tenant Estimate Aging automated notification can be configured to send an email or text message to sales representatives, other Rent Manager users, or tenants when an tenant estimate is a certain number of days old and has not expired or been set to a status of Approved or Rejected . This can be used to remind the tenant that the offer on the estimate is waiting for their response. Sales representatives may also be notified to alert them that they may want to check in with the tenant.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to send notifications to sales representatives at five intervals, but notify tenants only twice. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Tenant Estimate Aging .
The Automated Notification: Tenant Estimate Aging page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to specific Rent Manager users or user roles, property managers, sales representatives, or external contacts that do not have a user account.

 Tenant

 Creates a notification setup that can be sent to the tenant associated with the aging estimate.
 ...
 The number of days after the tenant estimate's Date when the notification should be sent. For example, if you would like the notification to be sent to when the estimate is ten days old, enter 10 .
 ...
 If the selected setup is under Notify Tenant , you can configure this notification setup to be sent to the tenant whose estimate is expiring and additional contacts associated with the tenant.

 In the Recipients tile header, click arrow_forward Add Tenant . In the Select contacts to include tile, select from the following options.
