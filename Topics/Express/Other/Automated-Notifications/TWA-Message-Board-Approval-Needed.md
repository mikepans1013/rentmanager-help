# TWA Message Board Approval Needed (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/TWA-Message-Board-Approval-Needed.htm

The TWA Message Board Approval Needed automated notification can be configured to send an email or text message to selected recipients when a tenant makes a new post on the Tenant Web Access (TWA) message board. This can be used to notify users that action is required to post the message on TWA.

 Related Preferences

 In order to utilize this notification, you must have the Show Message Board and Approval Required options enabled in system web preferences. For more information, refer to Tenant Web Access Message Board (System Web Preferences) .
 ...
 You can create a configuration so that users or external recipients can receive alerts. The configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select TWA Message Board Approval Needed .
The Automated Notification: TWA Message Board Approval Needed page displays.
...
The setup's details display.
 ...
 Include the manager of the property linked to the tenant whose message needs to be approved. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
