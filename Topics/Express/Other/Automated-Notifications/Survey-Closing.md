# Survey Closing (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Survey-Closing.htm

The Survey Closing automated notification can be configured to send an email or text message to tenants or users when a survey/poll posted to Tenant Web Access (TWA) is a certain number of days away from the End Date . The notification can be used to remind tenants that a survey is waiting for their response or alert a Rent Manager user so they can follow up with a tenant who has not yet answered.

 More Information

 This automated notification applies only to surveys posted to Tenant Web Access (TWA) (displays as Polls in the TWA web portal), and cannot be used for surveys that are manually sent via email and/or text message. For more information, refer to Add and Publish a Survey to Tenant Web Access (TWA) .
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to send reminders to tenants multiple times ahead of a survey's closure, but alert property managers only if a tenant has yet to respond two days before a survey closes. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Survey Closing .
The Automated Notification: Survey Closing page displays.
...
The setup's details display.
 ...
 The number of days before a survey's End Date that you want the automatic notification to send. For example, if you want alerts to be sent one week before the survey closes, enter 7 .
 ...
 If the selected setup is under Notify User , you can configure this notification setup to be sent to the user who published the survey, specific Rent Manager users or user roles, or external contacts that do not have a user accounts
 ...
 Include the user who published the survey to TWA.
 ...
 If the selected setup is under Notify Tenant , you can configure this notification setup to be sent to the tenant linked to the survey and additional contacts associated with the tenant.
 ...
 If enabled and the tenant is part of an account group, the notification is sent to the contact(s) marked as Primary for each tenant in the account group.
