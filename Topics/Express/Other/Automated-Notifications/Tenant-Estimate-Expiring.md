# Tenant Estimate Expiring (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Tenant-Estimate-Expiring.htm

The Tenant Estimate Expiring automated notification can be configured to send an email or text message to sales representatives, other Rent Manager users, or tenants when a tenant estimate is a certain number of days away from its expiration date. This can be used to remind the tenant that the offer on the estimate is waiting for their response. Sales representatives can also be notified to alert them that they may want to check in with the tenant.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Tenant Estimate Expiring .
The Automated Notification: Tenant Estimate Expiring page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 The number of days prior to the tenant estimate's Expiration Date when the notification should be sent. For example, if you would like the notification to be sent one week before the estimate expires, enter 7 .
