# Survey Response Received (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Survey-Response-Recieved.htm

The Survey Response Received automated notification can be configured to send an email and/or text message to property managers, other Rent Manager users, or external recipients to alert them that survey responses have been received. This notification can be used to remind the appropriate users, such as employees who monitor survey results or property managers to be kept in the loop of how tenants are responding at their properties, that tenants have responded to surveys.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to alert property managers when any survey is submitted, but only want to alert a maintenance user when responses are made to service-related surveys. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Survey Response Received .
The Automated Notification: Survey Response Received page displays.
...
The setup's details display.
 ...
 Each survey that generates a notification when tenants submit a response.

 Star Rating

 The star rating given as a response to the survey for which you want the automated notification to send. For example, if you want alerts to be sent when survey responses are rated with 2 stars, select 2 .
 ...
 Include the manager of the property that issued the survey. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
