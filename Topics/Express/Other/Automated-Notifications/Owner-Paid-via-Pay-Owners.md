# Owner Paid via Pay Owners (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Owner-Paid-via-Pay-Owners.htm

The Owner Paid via Pay Owners automated notification can be configured to send an email or text message to users and owners when distributions are paid out to owners. This allows you to keep both property managers and property owners on the same page regarding all equity distributions. These notifications are sent to the designated recipients when payments are made through either Pay Owners or Manually Pay an Owner .
 ...
 You can create multiple configurations so that different recipients can receive alerts. For example, you may wish to provide more information about the payment in the message sent to your accounts payable team than in the message sent to your owners. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Owner Paid via Pay Owners .
The Automated Notification: Owner Paid via Pay Owners page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the owners being paid.

 User

 Creates a notification setup that can be sent to specific Rent Manager users, user roles, or external contacts that do not have a user account.
 ...
 If the selected setup is under Notify User , you can configure this notification setup to be sent to specific Rent Manager users, user roles, or external contacts that do not have a user account.
 ...
 Include the manager of the property associated with the equity distribution. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.

 Add User or Role

 Include specific users or user roles.

 In the User / Role column, select each user or user role that receives this notification. Then in the Method column, select whether the associated user or user role receives the notification via Text , Email , or both Email And Text .

 Notify Owner

 If the selected setup is under Notify Owners , you can configure this notification setup to be sent to the owners being paid.

 In the Recipients tile header, click and select Add Owner .
