# Notice Date Set (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Notice-Date-Set.htm

The Notice Date Set automated notification can be configured to send an email or text message to users or external recipients once a tenant's Notice date has been set, which indicates when the tenant informed the property management company that they intend to move out. This can be useful in ensuring that all necessary inspections and move out reports are scheduled in a timely manner.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to notify property managers for all properties when a Notice date is set, but alert owners only when commercial tenants give notice. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Notice Date Set .
The Automated Notification: Notice Date Set page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the tenant and, optionally, tenant contact(s) whose notice date was set.

 Owner

 Creates a notification setup that can be sent to the owner(s) of the property where a notice date was set.
 ...
 To add an owner recipient, in the Recipients tile header, click arrow_forward Add Owner .
