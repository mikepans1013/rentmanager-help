# Owner Prospect Guest Card Submitted (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Owner-Prospect-Guest-Card-Submitted.htm

The Owner Prospect Guest Card Submitted automated notification can be configured to send an email and/or text message to Rent Manager users, owner prospects, or external recipients when owner prospect submits a guest card through your website. This could be used to prompt users to review the guest card, inform sales representatives of interested owner prospects, or thank owner prospects for their submission.
 ...
 You can create multiple configurations so that recipients can receive different alerts depending on the guest card template associated with the automated notification setup. For example, you may wish to notify sales representatives and property managers when commercial owner prospects submit guest cards, but alert only sales representatives when residential owner prospects submit guest cards. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Owner Prospect Guest Card Submitted .
The Automated Notification: Owner Prospect Guest Card Submitted page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the owner prospect who submitted the guest card.

 The setup's details display.

 -
 In the Filters tile's Guest Card Templates field, select each owner prospect-type guest card template that should generate alerts when a guest card is submitted. To include all current and future owner prospect-type guest cards in the notification, check *** <All Templates> .
 ...
 If the selected setup is under Notify User , you can configure this notification setup to be sent to specific Rent Manager users or user roles, or external contacts that do not have a user account.
 ...
 If the selected setup is under Notify Guest , you can configure this notification setup to be sent to the owner prospect who submitted the guest card.
 ...
 Include the owner prospect who submitted the guest card. This option can be configured to send only email notifications.
