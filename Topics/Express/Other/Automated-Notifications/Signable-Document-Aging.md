# Signable Document Aging (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Signable-Document-Aging.htm

The Signable Document Aging automated notification can be configured to send an email or text message to Rent Manager users or external recipients when a signable document has been waiting on a tenant or prospect's signature for a set amount of time. This can be used to streamline the process of requesting that signable documents be returned in a timely fashion.
 ...
 You can create multiple configurations so that recipients can receive alerts at different intervals. For example, you may wish to alert recipients when the document has been waiting for their signature for two, five, and seven days. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Signable Document Aging .
The Automated Notification: Signable Document Aging page displays.
...
The setup's details display.
 ...
 -
 Enter the number of Days old a signable document should be before a notification is sent. For example, if you would like the notification to be sent to when the document has been waiting for ten days, enter 10 .
