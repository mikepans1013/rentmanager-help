# Changed Contact Info via TWA (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Changed-Contact-Info-via-TWA.htm

The Changed Contact Info via TWA automated notification can be configured to send an email or text message to Rent Manager users or external recipients after a tenant changes their contact information through Tenant Web Access (TWA) .
 ...
 In order to utilize this notification, you must have the Allow tenants to change their account information option enabled in system web preferences. For more information, refer to Tenant Web Access Accounts (System Web Preferences) .
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to alert users working in the front office for all properties when tenants change their contact information, but notify property managers only for tenants at commercial properties. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Changed Contact Info via TWA .
The Automated Notification: Changed Contact Info via TWA page displays.
...
The setup's details display.
 ...
 Include the manager of the property linked to the tenant who changed their contact information. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
