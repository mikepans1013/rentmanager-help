# Architectural Request - Status Changed (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Architectural-Request-Status-Changed.htm

The Architectural Request - Status Changed automated notification can be configured to send an email to Rent Manager users, board/committee members, specific tenants, or external recipients when the status of an architectural request is updated in Rent Manager . This can be used to keep board and committee members informed on the request's progress, and can also be customized to trigger only when requests are updated to specific statuses.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to alert board and committee members at one property when the status is changed at all, but notify members at another property only when the status is changed to More Information Needed . Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Architectural Request - Status Changed .
The Automated Notification: Architectural Request - Status Changed page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the tenant who submitted the architectural request via Tenant Web Access (TWA) .
 ...
 The status(es) that should trigger a notification ( Approved , Denied , Canceled , More Information Needed ).
 ...
 If the selected setup is under Notify Tenant , you can configure this notification setup to be sent to the tenant who submitted the architectural request.

 In the Recipients tile header, click arrow_forward Add Submitting Tenant .
