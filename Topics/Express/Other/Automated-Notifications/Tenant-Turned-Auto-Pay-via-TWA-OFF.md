# Tenant Turned AutoPay via TWA OFF (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Tenant-Turned-Auto-Pay-via-TWA-OFF.htm

The Tenant Turned AutoPay via TWA OFF automated notification is configured to send an email or text message to selected recipients when tenants disable their AutoPay schedule in Tenant Web Access (TWA) . This can be used to notify users who handle tenant payments to expect a different form of payment for the upcoming month.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may only use TWA at some properties, and you want only the property managers at those properties to receive notifications. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Tenant Turned AutoPay via TWA OFF .
The Automated Notification: Tenant Turned AutoPay via TWA OFF page displays.
...
The setup's details display.
 ...
 Include the manager of the property linked to the tenant who disabled AutoPay. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
