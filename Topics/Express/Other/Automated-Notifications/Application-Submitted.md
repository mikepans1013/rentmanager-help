# Application Submitted (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Application-Submitted.htm

The Application Submitted automated notification can be configured to send an email or text message to property managers, other Rent Manager users, applicants, or owners after a prospect submits an Apply Now application. This can be used to prompt users to review the application, inform property managers of interested applicants, and provide email confirmation to the prospect that their application was received.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to notify property managers when applications are submitted at any property, but alert owners only when applications are submitted for commercial properties. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to   arrow_forward   Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Application Submitted .
The Automated Notification: Application Submitted page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the prospect who submitted the application.

 Owner

 Creates a notification setup that can be sent to the owner(s) of the property linked to the submitted application.
 ...
 Include the manager of the property linked to the prospect who submitted an application. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
 ...
 If the selected setup is under Notify Applicant , you can configure this notification setup to be sent to the prospect who submitted their application.
 ...
 Include the prospect who submitted the application. This option can be configured to send only email notifications.
 ...
 If the selected setup is under Notify Owner , you can configure this notification setup to be sent to the owner of the property for which an application was submitted.
