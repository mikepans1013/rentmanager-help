# Request for New Income Verification Link (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Income-Verification-Requested-From-Apply-Now.htm

The Request for New Income Verification Link automated notification can be configured to send an email or text message to users or external recipients when an applicant submits a request for a new income verification link via Tenant Web Access (TWA) . This can be used to notify users and/or external recipients that action is required to address the income verification link request, and removes back-and-forth communication for expired links by allowing the applicant to request a new one directly from Apply Now .
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Request for New Income Verification Link .
The Automated Notification: Request for New Income Verification Link page displays.
...
The setup's details display.
 ...
 -
 In the Filters tile, select each property that should receive alerts when the automated notification's conditions are met. To include all current and future properties, check the box in the drop-down header. Alternatively, select a property Group from the drop-down list.
 ...
 Include the manager of the property linked to the applicant who requested the new income verification link. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
