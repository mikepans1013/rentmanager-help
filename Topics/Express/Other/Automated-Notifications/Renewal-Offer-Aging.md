# Renewal Offer Aging (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Renewal-Offer-Aging.htm

The Renewal Offer Aging automated notification can be configured to send an email or text message to property managers, other Rent Manager users, tenants, or external recipients to notify them that a lease renewal offer has been opened for a specific number of days. This can be used to remind tenants of a pending renewal offer that needs their attention.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to create one notification that reminds tenants to review their open renewal offers and a separate one to ensure a property manager reaches out to the tenant in a timely manner. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Renewal Offer Aging .
The Automated Notification: Renewal Offer Aging page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the tenant associated with the aging renewal offer.
 ...
 Each property that should receive alerts when the automated notification's conditions are met.

 To include all current and future properties in the notification, check *** <All Properties> .
 ...
 The number of days after the renewal offer is sent when the notification should be sent. For example, if you would like the notification to be sent to when the renewal offer is ten days old, enter 10 .
 ...
 Include the manager of the property linked to the aging renewal offer. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
 ...
 If the selected setup is under Notify Tenant , you can configure this notification setup to be sent to the tenant whose renewal offer is expiring and additional contacts associated with the tenant.
