# Owner Signable Document Aging (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Owner-Signable-Document-Aging.htm

The Owner Signable Document Aging automated notification can be configured to send an email or text message to owners when a signable document has been waiting on their signature for a set amount of time. This can be used to streamline the process of requesting that signable documents be returned in a timely fashion.
 ...
 You can create multiple configurations so that recipients can receive alerts at different intervals. For example, you may wish to alert owners when the document has been waiting for their signature for two, five, and seven days. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Owner Signable Document Aging .
The Automated Notification: Owner Signable Document Aging page displays.
...
The setup's details display.
 ...
 -
 In the Filters tile's Days waiting for signature field, enter the number of days that should pass without the owner's signature before a notification is sent. For example, if you would like the notification to be sent when the document has been waiting for five days, enter 5 .
 ...
 In the Recipients tile header, click arrow_forward Add Signer to include owners who have not yet signed their documents.
