# Documents Published to TWA (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Documents-Published-to-TWA.htm

The Documents Published to TWA automated notification can be configured to send an email or text message to tenants after a signable document is published to their Tenant Web Access (TWA) portal. This can be used to notify tenants that a document is ready for review and signatures.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, tenants of certain properties may have less time to sign a document before it expires, so a configuration can be enabled to send specific instructions to the tenants of those properties. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Documents Published to TWA .
The Automated Notification: Documents Published to TWA page displays.
