# Owner Estimate Expiring (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Owner-Estimate-Expiring.htm

The Owner Estimate Expiring automated notification can be configured to send an email or text message to sales representatives, other Rent Manager users, or owners when an owner estimate is a certain number of days away from its expiration date. This can be used to remind the owner that the offer on the estimate is waiting for their response. Sales representatives can also be notified to alert them that they may want to check in with the owner.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Owner Estimate Expiring .
The Automated Notification: Owner Estimate Expiring page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the owner associated with the expiring estimate.
 ...
 The number of days prior to the owner estimate's Expiration Date when the notification should be sent. For example, if you would like the notification to be sent one week days before the estimate expires, enter 7 .
