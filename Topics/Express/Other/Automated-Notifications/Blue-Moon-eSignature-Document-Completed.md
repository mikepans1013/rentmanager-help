# Blue Moon eSignature Document Completed (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Blue-Moon-eSignature-Document-Completed.htm

The Blue Moon eSignature Document Completed automated notification can be configured to send an email or text message to users when a Blue Moon eSignature form is completed. This notification can help property managers who use Blue Moon leases ensure that their leasing workflows progress in a timely manner.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to have the notification sent to multiple users at one property, but only to the property manager at another. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Blue Moon eSignature Document Completed .
The Automated Notification: Blue Moon eSignature Document Completed page displays.

 -
 In the Notification Setups tile, select an existing setup from the list to edit its configuration or click Add to create a new setup.
The setup's details display.

 -
 In the Filters tile, select each property that should receive alerts when the automated notification's conditions are met. To include all current and future properties, check *** < All Properties > . Alternatively, select a property Group from the drop-down list.
 ...
 Include a person who does not have a user account in Rent Manager . Enter their information into the available field described below.
 ...
 Include the manager of the property that the Blue Moon document is associated with. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
