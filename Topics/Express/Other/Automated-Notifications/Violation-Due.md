# Violation Due (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Violation-Due.htm

The Violation Due automated notification can be configured to send an email or text message to users or non-users as the due date of a tenant's violation approaches. This can be used to remind property managers that a tenant must resolve their violation in a timely manner.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, some properties may not need to see this notification at all, and they can be excluded from the filter. Alternatively, managers at certain properties may need to perform a particular set of actions when a tenant's violation is due. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Violation Due .
The Automated Notification: Violation Due page displays.
...
The setup's details display.
 ...
 Days until violation due

 The number of days prior to a violation's Due Date when the notification should be sent. For example, if you would like the notification to be sent thirty days before the violation must be resolved, enter 30 .

 To send a notification after the violation was due, enter a negative number. For example, to send a notification five days after the violation's Due Date , enter -5 .

 -
 In the Scripting Type section, select which scripting class to use for any scripts used in the Email or Text tiles. The following scripting types are available.
 ...
 Use the Violation class as the default parent class for scripting in the notification messages. For more information, refer to Violation Class (Script) .
 ...
 Include the manager of the property linked to the violation. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
