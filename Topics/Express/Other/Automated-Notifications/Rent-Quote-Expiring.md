# Rent Quote Expiring (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Rent-Quote-Expiring.htm

The Rent Quote Expiring automated notification can be configured to send an email or text message to property managers, leasing agents, other users, or prospects when a prospect's rent quote is a certain number of days away from its expiration date. This can be used to remind the prospect that the rent quote offer is waiting for their response. Leasing agents can also be notified to alert them that they may want to check in with the prospect.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to send notifications to leasing agents at five intervals, but notify prospects only twice. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Rent Quote Expiring .
The Automated Notification: Rent Quote Expiring page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the prospect associated with the expiring rent quote.
 ...
 Days until rent quote expires

 The number of days prior to the prospect rent quote's Expiration date when the notification should be sent. For example, if you would like the notification to be sent one week before the rent quote expires, enter 7 .

 Exclude if prospect has a unit reserved

 Check this option if this notification is not to be sent for any prospects that have, on their Rent Quotes tile, a unit Status of Reserved .
 ...
 If the selected setup is under Notify User , you can configure this notification setup to be sent to specific Rent Manager users or user roles, leasing agents, or external contacts that do not have a user account.
 ...
 Include the manager of the property where the prospect's expiring rent quote is held. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
 ...
 If the selected setup is under Notify Prospect , you can configure this notification setup to be sent to the owner whose estimate is expiring.

 In the Recipients tile header, click arrow_forward Add Prospect . In the Select contacts to include section, select from the following options.
 ...
 The notification will be sent to all active contacts associated with the linked prospect account.

 Contact Type

 The notification will be sent only to active contacts with the selected contact type(s) on the prospect's View Contacts pop-up.

 Notify all tenants within account group

 If the prospect is part of an account group, the notification will be sent to the contact(s) marked as Primary for each tenant or prospect in the account group.

 Primary Contact

 The notification will be sent only to the contact marked as Primary on the prospect's View Contacts pop-up.
 ...
 If the selected setup is under Notify Prospect , you can opt to exclude individual prospects or any prospects that have a specified user defined field (UDF) value.

 In the Recipients tile header, click arrow_forward Exclude Prospect . Then on the Exclude Prospects pop-up, enter information into at least one of the available fields and click OK .
 ...
 Excludes specific prospect accounts. In the Prospect Exclusions section header, click . Then in the Prospect Search pop-up, search and select the prospect account you wish to exclude. Repeat for each prospect account that needs to be excluded.

 User Defined Field Exclusions

 Excludes recipients based on a prospect-type user defined field (UDF) value. In the first drop-down, select a UDF. Then in the second drop-down, enter or select the value. All prospect recipients with this value for the selected UDF do not receive the notification.
