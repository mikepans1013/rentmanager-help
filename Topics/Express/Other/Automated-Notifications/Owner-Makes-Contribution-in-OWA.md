# Owner Makes Contribution in OWA (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Owner-Makes-Contribution-in-OWA.htm

The Owner Makes Contribution in OWA automated notification can be configured to send an email or text message to property managers, other Rent Manager users, or external recipients when an owner makes a financial contribution in Owner Web Access (OWA) . This can be used to alert any relevant parties that the transaction has taken place so they can begin any next steps in the process.
 ...
 You can create a configuration so that users or external recipients can receive alerts. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Owner Makes Contribution in OWA .
The Automated Notification: Owner Makes Contribution in OWA page displays.

 -
 In the Notification Setups section, select the existing setup from the list to edit its configuration or click Add Notification to create a new setup.
The setup's details display.
 ...
 Include the manager of the property linked to the owner that made the contribution. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
