# Renewal Offer Status Updated (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Renewal-Offer-Status-Updated.htm

The Renewal Offer Status Updated automated notification can be configured to send an email or text message when the status of a lease renewal offer changes. This occurs when a Rent Manager user manually updates the status on the Renewal Offers page or when the offer receives a response from the tenant in the Tenant Web Access (TWA) portal. This can be used to notify users, tenants, and/or owners about the outcome of the lease renewal process.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to alert all property managers when renewal offers are accepted or denied, but notify the owners only for renewal offers at commercial properties. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Renewal Offer Status Updated .
The Automated Notification: Renewal Offer Status Updated page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to specific Rent Manager users or user roles, property managers, leasing agents, or external contacts that do not have a user account.

 Tenant

 Creates a notification setup that can be sent to the tenant and, optionally, tenant contact(s) who accepted or declined a renewal offer.

 Owner

 Creates a notification setup that can be sent to the owner(s) of the property where a renewal offer was accepted or declined.
 ...
 The current status of the lease renewal offer that should generate alerts.

 Offer Sent

 The lease renewal has been sent to the tenant and is awaiting their response.

 Needs Document

 The tenant has selected a lease term option in Tenant Web Access (TWA) .

 Waiting Tenant Signature

 If the renewal offer was delivered as a signable document, the offer is waiting for the tenant to sign the document for the lease renewal.

 Blue Moon Pending

 The Blue Moon signature status is changed to Pending .

 Blue Moon In Progress

 The Blue Moon signature status is changed to In Progress .

 Needs Landlord Signature

 If the renewal offer was delivered as a signable document, the offer is waiting for an owner or manager of the associated property to sign the document for the lease renewal.

 Blue Moon Signed

 The Blue Moon signature status is changed to Signed .

 Offer Accepted

 The lease renewal was approved by the tenant via Tenant Web Access (TWA) or manually by a Rent Manager user.

 Offer Accepted - Blue Moon Document Complete

 The renewal offer is accepted and the Blue Moon signature status is changed to Complete .

 Offer Accepted - Signable Document Completed

 If the renewal offer was delivered as a signable document, the tenant approved the lease renewal via TWA and signed the associated document.

 Offer Declined

 The lease renewal was rejected by the tenant via TWA or manually by a Rent Manager user.

 Offer Expired

 If the renewal offer was set to expire, the expiration date for the offer has passed before it was accepted by the tenant. The offer can no longer be completed and may need to be resent.

 Signable Document Expired

 If the renewal offer was delivered as a signable document set to expire, the expiration date of the document has passed before it was signed or accepted by the tenant. The offer can no longer be completed and may need to be resent.

 Blue Moon Expired

 The Blue Moon signature status is changed to Expired .
 ...
 If the selected setup is under Notify User , you can configure this notification setup to be sent specific Rent Manager users or user roles, property managers, or external contacts that do not have a user account.
 ...
 Include the manager of the property where the tenant is currently residing. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
 ...
 If the selected setup is under Notify Tenant , you can configure this notification setup to be sent to the tenant who accepted or rejected the renewal offer and additional contacts associated with the tenant.
