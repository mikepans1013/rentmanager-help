# Amenity Reservation Upcoming (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Amenity-Reservation-Upcoming.htm

The Amenity Reservation Upcoming automated notification can be configured to send an email or text message to property managers, other Rent Manager users, or tenants when they have a reservable amenity request date approaching in Tenant Web Access (TWA) . This can be used to alert users that the amenity needs to be ready before the reservation and remind tenants that their reservation is approaching.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to notify users three times before the amenity reservation, but notify tenants only once. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Amenity Reservation Upcoming .
The Automated Notification: Amenity Reservation Upcoming page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the tenant who made the amenity reservation.
 ...
 The number of days before an amenity reservation's Date that you want the automatic notification to send. For example, if you want alerts to be sent one week before the amenity reservation, enter 7 .
 ...
 Include the manager of the property linked to the amenity reservation. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
 ...
 If the selected setup is under Notify Tenant , you can configure this notification setup to be sent to the tenant who made the amenity reservation.
 ...
 If at least one recipient is configured to receive text notifications, the Text section displays with a default, system-generated message.
