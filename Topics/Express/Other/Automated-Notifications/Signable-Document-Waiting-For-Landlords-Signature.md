# Signable Document Waiting for Landlord's Signature (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Signable-Document-Waiting-For-Landlords-Signature.htm

The Signable Document Waiting For Landlord's Signature automated notification can be configured to send an email or text message to Rent Manager users or external recipients when a signable document has been returned by the signer and is waiting for landlord or manager approval. This can be used to streamline the process of verifying signable documents once they return from the signer.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to notify only the property manager that they need to sign a document at one property, but notify the manager and another specific user role for aging documents at another property. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Signable Document Waiting For Landlord's Signature .
The Automated Notification: Signable Document Waiting For Landlord's Signature page displays.
...
The setup's details display.
 ...
 -
 In the Filters tile, enter the number of Days waiting for landlord's signature that should pass before a notification is sent. For example, if you would like the notification to be sent when the document has been waiting for five days, enter 5 .
