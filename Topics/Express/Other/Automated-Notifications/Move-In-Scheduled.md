# Move In Scheduled (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Move-In-Scheduled.htm

The Move In Scheduled automated notification can be configured to send an email or text message to users, tenants, owners, or other external recipients as a future tenant's move in date approaches. This can help ensure that all necessary tasks related to the tenant's unit are completed in a timely manner and can also provide the tenant with updates about the unit before they move in.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to alert the property manager and leasing agent at all properties when a move in date is two weeks away, but notify tenants at multiple intervals. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Move In Scheduled .
The Automated Notification: Move In Scheduled page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the tenant and, optionally, tenant contact(s) whose move-in date is approaching.

 Owner

 Creates a notification setup that can be sent to the owner(s) of the property with approaching move-in dates.

 The setup's details display.

 - In the Filters tile, enter information into the following fields: Field Description
 Properties
 ...
 The number of days before a tenant's Move In date that you want the automatic notification to send. For example, if you want alerts to be sent one week before a tenant is scheduled to move in, enter 7 .

 Include prospects based on their expected move in date
 If checked, prospects with a Move In Date enter on their account receive alerts when the automated notification's conditions are met. Otherwise, notifications are sent only to tenants.
 ...
 If the selected setup is under Notify User , you can configure this notification setup to be sent specific Rent Manager users or user roles, property managers, leasing agents, or external contacts that do not have a user account.
 ...
 Include the manager of the property where the tenant is moving in. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.

 Add Leasing Agent

 Include the user selected on the tenant's details page in the Leasing Agent field.
 ...
 If the selected setup is under Notify Tenant , you can configure this notification setup to be sent to the tenant whose move-in date is approaching and additional contacts associated with the tenant.
 ...
 If the tenant is part of an account group, the notification will besent to the contact(s) marked as Primary for each tenant in the account group.
