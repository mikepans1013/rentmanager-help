# Tenant Turned AutoPay via TWA ON (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Tenant-Turned-Auto-Pay-via-TWA-ON.htm

The Tenant Turned AutoPay via TWA ON automated notification is configured to send an email or text message to selected recipients when tenants create or enable an AutoPay schedule in Tenant Web Access (TWA) . This can be used to notify users who handle tenant payments to expect a different form of payment for the upcoming month.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to alert only the property managers for one property when AutoPay is enabled, but also notify the accounting department for another property. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Tenant Turned AutoPay via TWA ON .
The Automated Notification: Tenant Turned AutoPay via TWA ON page displays.
...
The setup's details display.
 ...
 Include the manager of the property linked to the tenant who enabled AutoPay. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
