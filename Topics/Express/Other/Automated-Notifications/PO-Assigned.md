# Purchase Order Assigned (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/PO-Assigned.htm

The Purchase Order Assigned automated notification can be configured to send an email or text message to users when a purchase order is assigned to them. This can be used to notify users as soon as possible when a new purchase order is assigned to their username.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to notify only the assigned user at some properties, but at other properties you want to notify the assigned user and administrators. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Purchase Order Assigned .
The Automated Notification: Purchase Order Assigned page displays.
...
The setup's details display.
 ...
 Include the Rent Manager user who is now selected in the Assigned To field of the purchase order's details.
