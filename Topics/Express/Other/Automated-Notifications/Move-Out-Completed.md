# Move Out Completed (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Move-Out-Completed.htm

The Move Out Completed automated notification can be configured to send an email or text message to property managers, leasing agents, other Rent Manager users, or external recipients when a tenant is moved out of their unit. This can be useful in ensuring that all necessary paperwork and service issues related to the tenant move out are completed in a timely manner. In order for this notification to run, you must complete the Move Out wizard for the tenant.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to alert property managers for completed move outs at all properties, but notify leasing agents only for commercial properties. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Move Out Completed .
The Automated Notification: Move Out Completed page displays.
...
The setup's details display.
 ...
 Include the manager of the property where the move out was completed. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
