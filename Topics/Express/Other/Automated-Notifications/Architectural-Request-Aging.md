# Architectural Request Aging (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Architectural-Request-Aging.htm

The Architectural Request Aging automated notification can be configured to send an email to Rent Manager users, board/committee members, or external recipients after a predetermined number of days if the request has not been voted on by board or committee members or its status updated to Approved or Denied in Rent Manager .
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to send daily or weekly alerts to property managers as the request ages. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Architectural Request Aging .
The Automated Notification: Architectural Request Aging page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Each association-type property that should receive alerts when the automated notification's conditions are met. To include all current and future properties, check *** < All Properties > . Alternatively, select a property Group from the drop-down list.
 ...
 The number of days an unresolved architectural request must age before the notification is sent to the specified recipients. For example, if you enter 4 , then the notification is sent when an architectural request has not been Approved or Denied after four days.
 ...
 Include the manager of the association-type property linked to the aging architectural request. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
