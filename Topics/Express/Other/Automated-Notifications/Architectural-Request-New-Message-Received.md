# Architectural Request - New Message Received (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Architectural-Request-New-Message-Received.htm

The Architectural Request - New Message Received automated notification can be configured to send an email to property managers, other Rent Manager users, specific tenants, or external recipients when a user sends a message to the submitting tenant from the Architectural Request Detail pop-up or when a tenant sends a message on the request via Tenant Web Access (TWA) . This can be used to ensure that all parties remain up-to-date on communication regarding the architectural request.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to notify the property manager when a message is received for one property, but notify only designated users for another property. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Architectural Request - New Message Received .
The Automated Notification: Architectural Request - New Message Received page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Include the manager of the association-type property linked to the architectural request associated with the received message. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
