# Issue Closed (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Issue-Closed.htm

The Issue Closed automated notification can be configured to send an email or text message to users or external recipients when a service issue is closed in Rent Manager or rmAppSuite Pro . This can be used to notify users, such as property managers or maintenance team leaders, when the work on an issue is completed so they can follow up on the resolution.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to notify only users with the Maintenance role when a service issue is closed at certain properties, but notify tenants at all properties when an issue they are linked to is closed. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Issue Closed .
The Automated Notification: Issue Closed page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the tenant and, optionally, tenant contact(s) linked to the closed issue.

 Owner

 Creates a notification setup that can be sent to the owner(s) of the property linked to the closed issue.
 ...
 Each service issue category that should generate alerts when issues in those categories are closed and meet the automated notification's conditions.

 -
 In the Scripting Type section, select which scripting class to use for any scripts used in the Email or Text tiles. This section displays only for configurations under Notify Tenant or Notify Owner . The following scripting types are available.
 ...
 Use the Tenant class as the default parent class for scripting in the notification messages. For more information, refer to Tenant Class (Script) .

 This option is available only for Notify Tenant configurations.

 Owner

 Use the Owner class as the default parent class for scripting in the notification messages. For more information, refer to Owner Class (Script) .

 This option is available only for Notify Owner configurations.

 Service Manager

 Use the ServiceManager class as the default parent class for scripting in the notification messages. For more information, refer to Service Manager Class (Script) .
 ...
 Include the user assigned to the service issue at the time it is closed.

 Add Property Manager

 Include the manager of the property linked to the closed service issue. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
