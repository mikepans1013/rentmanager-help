# STR Reservation Added (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/STR-Reservation-Added.htm

The STR Reservation Added automated notification can be configured to send an email or text message to property managers, other Rent Manager users, guests, or external recipients when a short term rental (STR) reservation is created through Rent Manager or the Online Booking portal.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to notify property managers and specific users when an STR reservation is added via Online Booking, but alert only property managers when a reservation is added in Rent Manager . Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select STR Reservation Added .
The Automated Notification: STR Reservation Added page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to specific Rent Manager users or user roles, or external contacts that do not have a user account.

 Guest

 Creates a notification setup that can be sent to the guest whose reservation was added.

 The setup's details display.

 -
 At the top of the page, check or uncheck the Add to History option. If checked, this notification is automatically recorded as a history/note item on each recipient's account in Rent Manager each time this notification is sent out. If unchecked, the notification is sent to the recipient(s), but no history/note item is created. This option is available only for setups under Notify Guest .
 ...
 Each STR reservation source that should generate alerts when they are created. You can create notifications for reservations booked only by a user within Rent Manager , those created using only STR Online Booking , or Both .

 More Information

 If either STR Online Booking or Both is selected, the Booking Success Message entered in system web preferences is not sent, as creating automated notifications for either of these circumstances still provides a confirmation for the guest or user recipients. For more information, refer to STR Online Booking RV/Campground Settings (System Web Preferences) .
 ...
 If the selected setup is under Notify Guest , you can configure this notification setup to be sent to the STR guest.
 ...
 Include the manager of the property where a tenant's notice date was set. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
 ...
 If the selected setup is under Notify Guest , you can opt to exclude specific short term rental (STR) guests or any guests that have a specific user defined field (UDF) value.
