# Amenity Reservation Approval Needed (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Amenity-Reservation-Approval-Needed.htm

The Amenity Reservation Approval Needed automated notification can be configured to send an email or text message to property managers, other Rent Manager users, or external recipients when a reservable amenity request is submitted via Tenant Web Access (TWA) and is awaiting approval. This can be used to notify users that action is required to address the reservation request.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to notify the property manager and front office staff for one property, but notify only the property manager for another property. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Amenity Reservation Approval Needed .
The Automated Notification: Amenity Reservation Approval Needed page displays.
