# Payment Made via TWA (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Payment-Made-via-TWA.htm

The Payment Made via TWA automated notification can be configured to send an email or text message to selected recipients when tenants submit a payment through Tenant Web Access (TWA) , either manually or via AutoPay. This can be used to notify users that a payment was received so they can complete any necessary steps to process it, send a confirmation to the tenant that their payment was made successfully, and inform owners that tenants are making payments on time.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to alert Rent Manager users and tenants at all properties when a payment is made, but notify owners only for specific properties. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Payment Made via TWA .
The Automated Notification: Payment Made via TWA page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 Creates a notification setup that can be sent to the tenant and, optionally, tenant contact(s) who made the TWA payment.

 Owner

 Creates a notification setup that can be sent to the owner(s) of the property linked to the TWA payment.
 ...
 Include the manager of the property linked to the TWA payment. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
 ...
 If the selected setup is under Notify Tenant , you can configure this notification setup to be sent to the tenant linked who made the TWA payment and additional contacts associated with the tenant.
