# STR Reservation Payment Reminder (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/STR-Reservation-Payment-Reminder.htm

The STR Reservation Payment Reminder automated notification can be configured to send an email or text message to property managers, other Rent Manager users, guests, or external recipients when a short term rental (STR) reservation payment is due within a selected number of days.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. or example, you may wish to notify property managers at five intervals when an STR payment has not been submitted, but alert guests at only two intervals. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select STR Reservation Payment Reminder .
The Automated Notification: STR Reservation Payment Reminder page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 The number of days before the STR payment's due date, based on the applicable STR payment policy.

 If you enter a value higher than 1 , you can also check Send reminder once a day until balance is paid or until reservation is canceled rather than creating multiple setups for different numbers of days. For example, if you enter 10 and check this option, guests are first notified 10 days before the payment is due, then receive alerts each of the following nine days.
 ...
 If the selected setup is under Notify Guest , you can configure this notification setup to be sent to the STR guest who has not yet made a payment.
