# Architectural Request Submitted (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Architectural-Request-Submitted.htm

The Architectural Request Submitted automated notification can be configured to send an email to users or board/committee members when an association member submits a new architectural request. This informs committees of the submission and to prompts them to review the request. All parties designated as recipients receive notifications whether the request is submitted via Tenant Web Access or entered manually by a user in Rent Manager .
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, you may wish to inform the property manager for one property while informing the entire staff for another, or only send alerts to board members for a select group of properties. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Architectural Request Submitted .
The Automated Notification: Architectural Request Submitted page displays.
...
If you are adding a new setup, the Notification Type pop-up displays.
 ...
 -
 At the top of the page, check or uncheck the Add to History option. If checked, this notification is automatically recorded as a history/note item on each recipient's account in Rent Manager each time this notification is sent out. If unchecked, the notification is sent to the recipient(s), but no history/note item is created. This option displays only if Notify Board/Committee is selected.

 -
 In the Filters tile's Properties field, select each association-type property that should receive alerts when the automated notification's conditions are met. To include all current and future properties in the notification, check *** < All Properties > .
 ...
 Include the manager of the association-type property linked to the architectural request. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
