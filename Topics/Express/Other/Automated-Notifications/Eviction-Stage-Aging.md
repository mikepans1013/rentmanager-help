...
Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Eviction-Stage-Aging.htm

The Eviction Stage Aging automated notification can be configured to send an email or text message to property managers, other users, or external recipients to notify them that a tenant has been in a specific eviction stage for a specific number of days. This can be used to remind property managers of tenant evictions that need their attention.
 ...
 You can create multiple configurations so that recipients for different properties can receive alerts. For example, property managers have a list of tasks to perform when a tenant moves to a new stage. Each configuration displays in the Notification Setups list on the left of the page.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Eviction Stage Aging .
The Automated Notification: Eviction Stage Aging page displays.
...
The setup's details display.
 ...
 The number of days that should pass since the tenant's Stage Date Change before a notification is sent. For example, if you would like the notification to be sent one week after the Stage Date Change , enter 7 .

 Eviction Stage

 The category that identifies where in the eviction process the tenant is.
 ...
 Include the manager of the property the tenant is associated with . Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
