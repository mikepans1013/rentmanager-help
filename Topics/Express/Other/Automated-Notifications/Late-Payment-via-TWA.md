# Late Payment via TWA (Automated Notification)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Automated-Notifications/Late-Payment-via-TWA.htm

The Late Payment via TWA automated notification can be configured to send an email or text message to selected recipients when tenants submit a late payment via Tenant Web Access (TWA) . This can be used to notify Rent Manager users of a payment coming in after the due date and to ensure that any necessary late fees are added to the tenant account.
 ...
 -
 Go to arrow_forward Administration , then go to Automation arrow_forward General arrow_forward Automated Notifications and select Late Payment via TWA .
The Automated Notification: Late Payment via TWA page displays.
...
The setup's details display.
 ...
 Include the manager of the property linked to the tenant account that made the late payment. Email notifications are sent to the email address entered on the property's details page in the Email field. Text notifications are sent to the phone number marked as Default on the property's details page.
