# Profit & Loss 12 Month Recap (Report)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Reports/Profit-Loss-12-Month-Recap.htm

The Profit & Loss 12 Month Recap report displays income and expense general ledger (GL) accounts for selected properties for up to a 12-month period, allowing you to compare the profitability of your portfolio across multiple months. This P&L report also displays the net income to track the overall financial impact in each month.
 ...
 Additionally, on the Reports tab, you must have access to Profit & Loss 12 Month Recap .

 For more information, refer to Control User Access .

 To view the Profit & Loss 12 Month Recap report, do the following:

 -
 Go to arrow_forward Financial Statements arrow_forward Profit & Loss arrow_forward Profit & Loss 12 Month Recap .
The Reports: Profit & Loss 12 Month Recap page displays.
 ...
 If the Restrict by owner contract dates report option is checked and the Date Rage begins before and/or ends after the active owner contract, the report header displays only the dates where the active owner contract and the Date Range overlap. Filtered results display an asterisk (*) after the Date Range .

 Income

 A row for each general ledger account with the Income type that displays the ending income account balances for all property transaction activity that occurred during the Date Range . Income accounts are grouped together by type and provide subtotals for each grouping.

 Expense

 A row for each general ledger account with the Expense type that displays the ending expense account balances for all property transaction activity that occurred during the Date Range . Expense accounts are grouped together by type and provide subtotals for each grouping.

 Net Income

 This section of the report displays the net income for each month (calculated by subtracting the total expenses from the total income), followed by the net income for all properties in the report. Net income is calculated using the following formula:
 ...
 The name and number for income and expense GL accounts with transaction activity during the Date Range .

 Month Columns

 The balance for the income and expense GL accounts with activity within each month during the Date Range .
 ...
 The Net Income Summary subreport displays the income, expense, and net income for each month as well as the totals for all the selected properties.
