# Statement (Option 2 Report)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Reports/Tenant-Statement-Option-2.htm

The Statement (Option 2) report displays transaction information for selected tenants across a date range, including the totals for charges, credits, and the current balance due. Because the statement is intended to be mailed to tenants at the end of each billing period, it fits on an 8.5x11 sheet of paper when printed and includes a mailing address header. It generates information similar to the Statement - 8.5x11 report but has a different visual presentation allowing for color in the text, section dividers, and more.
 ...
 To view the Statement (Option 2) report, do the following:

 -
 Go to arrow_forward Tenant Statements arrow_forward General arrow_forward Statement (Option 2) .
The Reports: Statement (Option 2) page displays.
 ...
 The Statement (Option 2) report can also be generated directly from the Tenants page by doing the following:
...
The Run Reports pop-up displays.
 ...
 -
 In the Select a Report drop-down list, select Statement (Option 2) .
...
The Reports: Statement (Option 2) page displays.
 ...
 -
 To select multiple tenants, click Bulk Actions arrow_forward Statements , check the box next to each tenant, then select Tenant Statement (Option 2) . When finished, click Run Report .
The Reports: Statement (Option 2) page displays.
 ...
 The tenant address can be found on the tenant's details page. The property address displayed here is the Billing address found on the Primary Address tile on the property's details page. To display the property name on the mailing header, you must enter a name in the Billing Name 1 field on the Other Information tile on the property's details page. For more information, refer to Property Details (Page) .
 ...
 Comments entered in report options display below the Balance Due amount.
 ...
 The Amount of each transaction as displayed on the tenant's View Transactions page.
 ...
 Any available credits on the tenant account that can be applied toward open charges. This section displays only if there are outstanding credits on a tenant's account.
 ...
 The total amount of the credit available for the tenant.

 Used in prior billing period

 The amount of the credit that was applied toward a charge in the previous billing period.

 Used in current billing period

 The amount of the credit that was applied toward a charge in the current billing period on this statement.

 Remaining

 The total remaining amount of the credit that can still be applied to a charge.
