# CAM Budgeted Expense Breakdown (Report)

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Reports/CAM-Budgeted-Expense-Breakdown.htm

The CAM Budgeted Expense Breakdown report displays the total budgeted property common area maintenance (CAM) expense over the report date range, the selected commercial tenant's share of the budgeted expense (minus any Cap Amount ), and the adjusted monthly amount of the tenant's share.
 ...
 Additionally, on the Reports tab, you must have access to CAM Budgeted Expense Breakdown .

 For more information, refer to Control User Access .

 To view the CAM Budgeted Expense Breakdown report, do the following:

 -
 Go to arrow_forward Accounting arrow_forward Commercial arrow_forward CAM Budgeted Expense Breakdown .
The Reports: CAM Budgeted Expense Breakdown page displays.
...
For more information, refer to Set Up a CRE Charge Type .
 ...
 The total amount of CAM expenses entered in the budget that the selected CAM charge type recovers for the property during the selected Cap Term .

 Property Expense Less Base

 The total budgeted amount of CAM expense for the property minus any base amount entries on the lease's Charge Type Setup tab. For example, if the base amount is set to $1,000 for the specified charge type and the cap term is monthly, Rent Manager multiplies the base amount by 12 (months), and the monthly base amount is deducted from the property expense.
 ...
 The Summary subreport details the tenant’s share of the budgeted property’s expense, the total amount that can be charged, and the new amount that is charged to the selected commercial tenant each month.
 ...
 The selected tenant’s total share of all the budgeted property expense accounts displayed in the report results. A tenant's share is calculated each month using the following formula:
 ...
 If the Cap Amount row is greater than the Total Tenant's Share , the Cap Amount becomes the New Monthly Amount . If the Cap Amount is less than the Total Tenant's Share , the Total Tenant's Share amount displays instead.

 New Monthly Amount

 If the Cap Amount is greater than the Total Tenant Share , the Cap Amount displays. If the Cap Amount is less than the Total Tenant's Share , the Cap Amount divided by the number of months in the fiscal year.
