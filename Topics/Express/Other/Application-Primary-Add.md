# Add a Primary Application Template

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Application-Primary-Add.htm
...
Apply Now allows prospective tenants to submit an online application and automates the process of receiving rental applications, collecting application fees, and screening prospects. To ensure that these applications are sent to the appropriate property and provide the necessary information to make a decision, you need to create application templates.
 ...
 There are two kinds of application templates: Primary and Sub-Application . Primary application templates are used to collect information for the main contact for a unit, while subapplication templates are used for additional associated coapplicants such as roommates and guarantors. Applicants send subapplications to any necessary contacts to ensure that all of the information is consolidated in one place.
 ...
 Step 1: Create a Primary Application Template
 ...
 There are two ways to create a primary application template: build a new template from scratch or copy any existing Primary - or Sub-Application -type template to use as a basis. Each option is described below.

 Option 1: Manually Add a Template

 To manually create a new primary application template, do the following:
...
The Add Application Template pop-up displays.
 ...
 -
 In the Template Type drop-down list, leave Primary selected.
...
The Copy pop-up displays.
 ...
 -
 If you copied a Primary -type template, select As a new primary application template . If you copied a Sub-Application -type template, select Convert to a new primary application template .
 ...
 In the General tile, enter the user- and prospect-facing details for the template, choose property defaults, and establish requirements for prospects to create and submit applications in the following fields:
 ...
 The unique name of the application template that displays internally for Rent Manager users.

 Display Name

 The name of the application template that displays to applicants on Apply Now . This name displays only if there are multiple application templates available for the same property.

 Description

 A description of the application that displays to applicants on Apply Now , intended to help them make a selection when multiple applications are available.

 Financial Property

 The property that receives application fees for applications submitted using this template. This may be different from the property to which the application is submitted. For example, if the property management company collects all application payments regardless of property, you may select the management company's property here.

 Related Preferences

 To have this application template always use the default Financial property established in system web preferences, select <System Web Preferences Default> . If the system preference is changed, applications submitted after the change will have their fees received by the new property.

 For more information, refer to Tenant Web Access Apply Now ePay (System Web Preferences) .

 Default Property

 The property to assign new prospects when they submit an application using this template. This may be different from the property to which the application is submitted. For example, if the property management company tracks all submitted applications regardless of property, you may select the management company's property here.

 Related Preferences

 To have this application template always use the default Default Property established in system web preferences, select <System Web Preferences Default> . If the system preference is changed, applications submitted after the change will have their fees received by the new property.

 For more information, refer to Tenant Web Access Apply Now General (System Web Preferences) .

 Selection Required

 Require a prospect to select the Property and/or Unit/Unit Type they wish to apply to.

 Once you have filled out this tile, click Save to create the primary application template.
 ...
 Step 6: Create a Sub-Application Template

 If multiple contacts need to complete an application, such as additional occupants, guarantors, or other coapplicants, you also need to create a subapplication template for each applicant type. For more information on adding subapplication templates, refer to Add a Sub-Application Template .

 Step 7 : Add Additional Applicant Section

 Once you create a subapplication template, you can add an Additional Applicant section into the primary application. This allows the applicant to distribute the necessary subapplications via email, which are then linked with your primary applicant's prospect account with subapplicants added as contacts. It also lets you establish default settings for any subapplications.
