...
Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Importing/Charges.htm
...
The importing tool allows you to pull large amounts of data into Rent Manager at one time via a comma-separated values (CSV) file or other file type that uses supported delimiters. The file is first prepared outside Rent Manager , using a program such as Microsoft Excel or Notepad, which contains rows of data representing the information that matches to designated Rent Manager fields. This greatly reduces the amount of time spent on data entry, especially when first setting up your Rent Manager database, or during routine additions that require a lot of new data, such as acquiring a new property.
 ...
 When importing charges, you can add new charges to existing tenants in Rent Manager .
 ...
 Data in your import file should be organized with each row as a separate entity and each column containing information relevant to that entity. For example, an import of charge data is displayed with each row representing a different charge and each column representing information about that charge.

 Required Fields

 In order to import charges, the Amount field is required to have a value entered in the data file.

 Charge Field Descriptions

 You can import values into any Rent Manager charge field, including user defined fields (UDFs). As long as your import file meets the field requirements above, you can include as many or as few additional fields of your choice.
 ...
 The system-generated account number of the tenant or prospect, as shown on the account's details page in the Account# field.
 ...
 The Short Name of the property at which the tenant resides, as displayed on the property's details page.

 Reference

 A reference number or identifier for the charge.

 Unit Name

 The Name of the unit at which the tenant resides, as displayed on the unit's details page.

 User Defined Field

 This column should be used only if you are matching to tenant and prospect accounts based on a user defined field (UDF) value. During Step 2: Begin the Import , in the Match to Tenants by section, the option Use the following value from a User Defined Field option must be selected to determine which UDF to use for matching.

 Unlike other importing tools, this UDF column does not accept a selection for Handling . The values in this column are instead used to match the charges to tenants and prospects based on the value entered in the UDF selected in the Match to payments by section.

 Step 2: Begin the Import

 To import charges, do the following:

 -
 Go to arrow_forward Administration , then go to Import arrow_forward Import Records arrow_forward Import Charges .
The Import Charges Settings pop-up displays.
 ...
 -
 In the Match to Tenants by section, specify how Rent Manager matches the accounts in your import file to the existing tenant or prospect records. The available options are described below.
 ...
 Use the tenant or prospect's system-generated account number, as shown on the account's details page in the Account# field, as the matching identifier. The Account Number field must be included in the import file.

 Use Property Short Name and Unit supplied in the import file

 Use the Property Short Name (as displayed on the property's details page) and the unit Name (as displayed on the unit's details page) as the matching identifiers. The Property Short Name and Unit Name fields must be included in the import file.

 With this option, Rent Manager matches to the unit and not to the specific tenant, meaning the charge is assigned to the current tenant of that unit. If no current tenant exists, the import fails for that specific record.

 Use Unit supplied in the import file and the following Property

 Use the unit Name (as displayed on the unit's details page) and the property selected from the drop-down list as matching identifiers. The Unit field must be included in the import file.

 With this option, Rent Manager matches to the unit and not to the specific tenant, meaning the charge is assigned to the current tenant of that unit. If no current tenant exists, the import fails for that specific record.

 Use the following value from a User Defined Field

 Use the value from a selected tenant-type user defined field (UDF) as the matching identifier. The import file must have a column for the UDF's values.
 ...
 The date that displays on tenant transactions for the charges.

 Charge Type

 The default charge type that Rent Manager uses if there is no Charge Type data in the file.

 Comment

 A default comment to be added to each imported charge.
 ...
 If checked, Rent Manager automatically creates an invoice for each imported charge.

 Invoice Comment

 The default comment to be added to each created invoice.

 -
 Click Next .
The pop-up closes and the Import Charges page displays.
