...
Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Importing/Unit-Types.htm
...
The importing tool allows you to pull in large amounts of data into Rent Manager at one time via a comma-separated values (CSV) file or other file type that uses supported delimiters. The file is first prepared outside Rent Manager using a program such as Microsoft Excel or Notepad, which contains rows of data representing the information that matches to designated Rent Manager fields. This greatly reduces the amount of time spent on data entry, especially when first setting up your Rent Manager database, or during routine additions that require a lot of new data, such as acquiring a new property.
 ...
 When importing unit types, you can add new unit types in Rent Manager .
 ...
 Data in your import file should be organized with each row as a separate entity and each column containing information relevant to that entity. For example, an import of unit type data is displayed with each row representing a different unit type and each column representing information about that unit type.

 Required Fields

 In order to import unit types, the Unit Type Name field must always have values entered for all unit types.

 Field Descriptions

 You can import values into any Rent Manager unit type field. As long as your import file meets the field requirements above, you can include as many or as few additional fields of your choice.
 ...
 The number of bathrooms in all units of this type. This is imported to the unit type's details page in the Bathrooms field.

 Bedrooms

 The number of bedrooms in all units of this type. This is imported to the unit type's details page in the Bedrooms field.

 Description

 A brief summary of each unit type. This is imported to the unit type's details page in the Description field.

 Other Rentable Item

 Items (e.g., storage units, garages, or parking spaces) that do not have marketing information and/or should not be included in occupancy-based reporting or calculations.

 Properties Short Name

 The Short Name of each property at which this unit type is active. Multiple short names can be separated by commas and no spaces (e.g., RIVER,123ST,101-AB ).

 Unit Type Name

 The unique name of the unit type.

 Step 2: Begin the Import

 To import unit types, do the following:

 -
 Go to arrow_forward Administration , then go to Import arrow_forward Import Records arrow_forward Import Unit Types .
The Import Unit Types Settings pop-up displays.
 ...
 -
 In the Default Values section, enter a default description in the Comments field. This value is automatically applied to any blank rows for the Description column in the import file, or to all rows if there is no Description column.

 -
 Click Next .
The pop-up closes and the Import Unit Types page displays.
