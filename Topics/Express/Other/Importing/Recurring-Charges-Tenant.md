# Import Tenant Recurring Charges

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Importing/Recurring-Charges-Tenant.htm
...
The importing tool allows you to pull large amounts of data into Rent Manager at one time via a comma-separated values (CSV) file or other file type that uses supported delimiters. The file is first prepared outside Rent Manager , using a program such as Microsoft Excel or Notepad, which contains rows of data representing the information that matches to designated Rent Manager fields. This greatly reduces the amount of time spent on data entry, especially when first setting up your Rent Manager database, or during routine additions that require a lot of new data, such as acquiring a new property.
 ...
 This topic covers importing tenant recurring charges. If you are importing recurring charges for another entity, refer to one of the topics listed below.

 -
 For importing unit recurring charges, refer to Import Unit Recurring Charges .

 -
 For importing unit type recurring charges, refer to Import Unit Type Recurring Charges .

 -
 For importing property recurring charges, refer to Import Property Recurring Charges .

 When importing recurring charges for tenants , you can add new recurring charges or update existing recurring charges.
 ...
 In order to import new or update existing tenant-level recurring charges, the following fields must always have values entered.
 ...
 The tenant's system-generated account number as shown on the account's details page in the Account# field.
 ...
 The property's system-generated ID number used to identify the property. The ID is located by using the PropertyID function in scripting. For more information, refer to Property Class (Script) .
 ...
 The value for the any tenant-type user-defined field (UDF) whose value is used to match tenant records. During Step 2: Begin the Import , in the Match to Tenants by section, the Use the following value from a User Defined Field option must be selected to determine which UDF to use for matching. Then, when importing, you must select the UDF you are using to match in the Handling drop-down list. For more information, refer to User Defined Fields (Page) .
...
The Import Recurring Charges Settings pop-up displays.
 ...
 -
 In the Select Import Recurring Charge Type field, select Tenant .
 ...
 - In the Match to Tenants by section, specify how Rent Manager matches the information in your import file to the existing tenant records. The available options are described below. Option Description
