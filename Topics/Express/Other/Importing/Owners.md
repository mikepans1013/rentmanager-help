...
Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Importing/Owners.htm
...
The importing tool allows you to pull large amounts of data into Rent Manager at one time via a comma-separated values (CSV) file or other file type that uses supported delimiters. The file is first prepared outside Rent Manager , using a program such as Microsoft Excel or Notepad, which contains rows of data representing the information that matches to designated Rent Manager fields. This greatly reduces the amount of time spent on data entry, especially when first setting up your Rent Manager database, or during routine additions that require a lot of new data, such as acquiring a new property.
 ...
 When importing owners, you can choose whether you are adding new owner data, or updating owner data that already exists in Rent Manager .
 ...
 Data in your import file should be organized with each row as a separate entity and each column containing information relevant to that entity. For example, an import of owner data is displayed with each row representing a different owner and each column representing information about that owner.

 Required Fields

 In order to import owners, some fields are required to have a value entered in the data file. The fields that are required depend on whether you are adding new owners, or updating the information for owners already in Rent Manager .
 ...
 For adding new owners to Rent Manager , the Owner Name field must have values entered.

 Update Existing Owners

 For updating owners already in Rent Manager , at least one of the following fields must have a value entered:
 ...
 You can import values into any Rent Manager owner field, including user-defined fields (UDFs). As long as your import file meets the field requirements above, you can include as many or as few additional fields of your choice.

 The following fields can be added as columns to import owner information:
 ...
 The name used when writing checks for the owner, to display on the owner's details page Miscellaneous tile.

 City

 The city as listed on the owner details page's Addresses tile.

 Comment

 A note providing additional information about the owner.

 Default Address Type

 One of the existing address types to be the default address type for the owner.

 Display

 The Display Color for the owner, to display on the owner's details page Miscellaneous tile.
 ...
 The primary Email address for contacting the owner to display on the owner's details page.

 Owner ID

 The system-generated account number for this owner account. This number can be obtained using the OwnerID script function. For more information, refer to Owner Class (Script) .

 This field is used only for updating existing owners. If adding new owners, this column is skipped and Rent Manager automatically generates a property ID for each property during the import.

 Owner Name

 The name that appear on reports and other places throughout Rent Manager .

 Phone Number

 One of the phone numbers for the owner. If an owner has multiple phone numbers, create a separate column for each type of phone number.

 Postal Code

 The postal or zip code as listed on the owner details page's Addresses tile.

 State

 The state as listed on the owner details page's Addresses tile.

 Street

 The street as listed on the owner details page's Addresses tile.

 Street2

 If applicable, enter a second address line for the owner's address.

 TaxID

 The ID used for tax reporting purposes, to display on the owner's details page General tile.

 User Defined Field

 Any owner-type user-defined fields (UDFs) can be added as their own column. Then enter the values for that UDF for each owner. For more information, refer to Owner-Type User Defined Fields (Pop-Up) .

 Step 2: Begin the Import

 To import owners, do the following:

 -
 Go to arrow_forward Administration , then go to Import arrow_forward Import Records arrow_forward Import Owners .
The Import Owners Settings pop-up displays.
 ...
 Create new owner accounts in Rent Manager .

 Update Existing Owner

 Replace or add information for owners that are already in Rent Manager .

 To remove information from any fields on existing owners where there is an empty cell in the import file, check Replace data with blanks . Otherwise, the information for that field on the owner is unchanged.
 ...
 Match the data to the system-generated account number for the owner. This number can be obtained using the OwnerID script function. Each owner record in the import file is required to have the Owner ID field specified.

 Use the following value from a User Defined Field

 Match the data to a user defined field (UDF), then select the UDF to match to for each owner. The import file must have a column for the UDF's values.

 -
 Click Next .
The pop-up closes and the Import Owners page displays.
