...
Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Importing/Jobs.htm
...
The importing tool allows you to pull large amounts of data into Rent Manager at one time via a comma-separated values (CSV) file or other file type that uses supported delimiters. The file is first prepared outside Rent Manager , using a program such as Microsoft Excel or Notepad, which contains rows of data representing the information that matches to designated Rent Manager fields. This greatly reduces the amount of time spent on data entry, especially when first setting up your Rent Manager database, or during routine additions that require a lot of new data, such as acquiring a new property.
 ...
 When importing jobs, you can choose whether you are adding new job data, or updating job data that already exists in Rent Manager .
 ...
 Data in your import file should be organized with each row as a separate entity and each column containing information relevant to that entity. For example, an import of job data is displayed with each row representing a different job and each column representing information about that job.

 Required Fields

 In order to import jobs, some fields are required to have a value entered in the data file. The fields that are required depend on whether you are adding new jobs, or updating the information for jobs already in Rent Manager .
 ...
 For adding new jobs to Rent Manager , all of the following fields must have values entered:
 ...
 You can import values into any Rent Manager job field, including user defined fields (UDFs). As long as your import file meets the field requirements above, you can include as many or as few additional fields of your choice.

 The following fields can be added as columns to import job information:
 ...
 If applicable, the date on which the job was completed.

 Expected End Date

 The date on which the job is expected to be completed.

 Job Type

 The exact name of the applicable job type to categorize this job. For more information, refer to Job Types (Page) .
 ...
 The abbreviated name of the job.

 Start Date

 The date on which the job began or is expected to begin.

 Type

 If this job is linked to a property, unit, or asset, enter the full Name of the associated property, unit, or asset. In Step 4: Match Data Fields , select the applicable entity for the Type column in the Handling field: Property Job Link Type , Unit Job Link Type , or Asset Job Link Type .

 When importing the entity Type information, only one option can be selected per import. For example, if you are importing jobs that are linked to properties and jobs that are linked to units, you must create two separate import files: one import file for the asset-linked jobs and one import file for the property-linked jobs. Then perform the import process separately for each file.

 Warning

 If there are multiple units with the same name in Rent Manager , the job information cannot be imported with links to those units even if the units are at different properties. It is recommended that you import jobs linked to those units without a value in the Type column and manually link those jobs to the proper unit in Rent Manager after importing. For more information, refer to Job Details (Page) .

 User Defined Field

 Any job-type user defined fields (UDFs) can be added as their own column. For more information, refer to Job-Type User Defined Fields (Pop-Up) .
 ...
 -
 Go to arrow_forward Administration , then go to Import arrow_forward Import Records arrow_forward Import Jobs .
The Import Jobs Settings pop-up displays.
 ...
 Create new job accounts in Rent Manager .

 Update Existing Jobs

 Replace or add information for jobs that are already in Rent Manager .

 To remove information from any fields on existing jobs where there is an empty cell in the import file, check Replace data with blanks . Otherwise, the information for that field on the job is unchanged.

 If you selected Add New Jobs , the pop-up closes and the Import Jobs page displays. Proceed to Step 3: Configure Import File for Rent Manager .

 -
 If you selected Update Existing Jobs , select a Match to jobs by option to specify how Rent Manager matches the properties in your import file to the existing property records. The available options are described below.

 Option
 Description

 Use Job Name supplied in the import file

 Match the data to the name of the job as it is entered on each job's details page in the Name field.

 Use Property Short Name supplied in the import file

 Match the data to the abbreviated name of the job as it is entered on each job's details page in the Short Name field.

 Use the following value from a User Defined Field

 Match the data to a user defined field (UDF), then select the UDF to match to for each job. The import file must have a column for the UDF's values.

 -
 Click Next .
The pop-up closes and the Import Jobs page displays.
 ...
 For example, if the column has data about the dates on which jobs begin, select Start Date .
