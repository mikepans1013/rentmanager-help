# Import Unit Recurring Charges

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Importing/Recurring-Charges-Unit.htm
...
The importing tool allows you to pull large amounts of data into Rent Manager at one time via a comma-separated values (CSV) file or other file type that uses supported delimiters. The file is first prepared outside Rent Manager , using a program such as Microsoft Excel or Notepad, which contains rows of data representing the information that matches to designated Rent Manager fields. This greatly reduces the amount of time spent on data entry, especially when first setting up your Rent Manager database, or during routine additions that require a lot of new data, such as acquiring a new property.
 ...
 This topic covers importing unit recurring charges. If you are importing recurring charges for another entity, refer to one of the topics listed below.
 ...
 When importing recurring charges for units , you can add new recurring charges or update existing recurring charges.
 ...
 In order to import new or update existing unit-level recurring charges, the following fields must always have values entered.
 ...
 The Short Name of the property at which the unit is located, as displayed on the property's details page.
 ...
 The system-generated Unit ID number, which is used to identify the unit to which recurring charges are being imported.

 Unit Name

 The Name of the unit as displayed on the unit's details page.

 User Defined Field

 The value for the any unit-type user-defined field (UDF) whose value is used to match unit records. During Step 2: Begin the Import , in the Match to Units by section, the Use the following value from a User Defined Field option must be selected to determine which UDF to use for matching. Then, when importing, you must select the UDF you are using to match in the Handling drop-down list. For more information, refer to User Defined Fields (Page) .
...
The Import Recurring Charges Settings pop-up displays.
 ...
 -
 In the Select Import Recurring Charge Type field, select Unit .
 ...
 - In the Match to Units by section, specify how Rent Manager matches the information in your import file to the existing unit records. The available options are described below. Option Description
 Use Unit ID supplied in the import file
 Use the unit's system-generated ID number used to identify the property. The Property ID field must be included in the import file.
 ...
 Use the value from a selected unit-type user defined field (UDF) as the matching identifier. The import file must have a column for the UDF's values.
