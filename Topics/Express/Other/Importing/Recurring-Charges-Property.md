# Import Property Recurring Charges

Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Importing/Recurring-Charges-Property.htm
...
The importing tool allows you to pull large amounts of data into Rent Manager at one time via a comma-separated values (CSV) file or other file type that uses supported delimiters. The file is first prepared outside Rent Manager , using a program such as Microsoft Excel or Notepad, which contains rows of data representing the information that matches to designated Rent Manager fields. This greatly reduces the amount of time spent on data entry, especially when first setting up your Rent Manager database, or during routine additions that require a lot of new data, such as acquiring a new property.
 ...
 This topic covers importing property recurring charges. If you are importing recurring charges for another entity, refer to one of the topics listed below.
 ...
 When importing recurring charges for properties , you can add new recurring charges or update existing recurring charges.
 ...
 In order to import new or update existing property-level recurring charges, the following fields must always have values entered.
...
The Import Recurring Charges Settings pop-up displays.
 ...
 -
 In the Select Import Recurring Charge Type field, select Property .
 ...
 - In the Match to Properties by section, specify how Rent Manager matches the information in your import file to the existing property records. The available options are described below. Option Description
 Use Property ID supplied in the import file
 Use the property's system-generated ID number used to identify the property. The Property ID field must be included in the import file.

 Use Property Short Name supplied in the import file
 Use the property's abbreviated name associated with the property. The Property Short Name field must be include in the import file.

 Use the following value from a User Defined Field
 Use the value from a selected property-type user defined field (UDF) as the matching identifier. The import file must have a column for the UDF's values.
