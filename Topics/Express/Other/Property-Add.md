...
Source: https://rmxhelp.rentmanager.com/Topics/Express/Other/Property-Add.htm
...
Most people think of properties in terms of real estate: either land or a physical building. In Rent Manager , the definition of a property applies to any entity with its own financial data (i.e., generates revenue and incurs expenses). This means you should create a property in Rent Manager for each financial entity.
 ...
 For example, if you have an apartment complex where each building has its own finances that are tracked separately, you would create each building as an individual property. However, if the all buildings at the apartment complex share financial data tracking, you would create only one property and add a unit to the property for each individual apartment unit from all buildings.

 This topic covers adding properties to your database, including but not limited to the following types:
 ...
 By default, the financial entities in your Rent Manager database are called Properties . You can customize the entity name in system preferences to meet your business needs. For example, if you own only properties that are homes , you can change Property to House . This updates the entity name for your entire Rent Manager database, so be sure to choose an entity name that works best for all your business purposes. For more information, refer to Entity Types (System Preferences) .
...
The Properties page displays.
 ...
 The category that describes what type of property it is, such as Apartment , Duplex , or Single Family .

 More Information

 If you are adding a commercial property, such as a retail shop or office, refer to Add a Commercial Property for more information.

 If you are adding a short term rental (STR) property, such as a vacation home or campground, refer to Add a Short Term Rental Property for more information.
 ...
 This is useful for properties that have only one unit, such as a single-family home or a standalone storefront, because properties require at least one unit in Rent Manager to properly track and record leases, finances, and other important information. For properties that have multiple units, such as duplexes and apartments, you can add multiple units to the property all at once after the property is created.
